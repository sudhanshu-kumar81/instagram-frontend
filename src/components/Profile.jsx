...
const Profile = () => {
  const [followerOrUnfollowList, setFollowerOrUnfollowList] = useState('')
  const setFollowersControls = (e) => {
    e.preventDefault();
    followerOrUnfollowList === 'followers' ? setFollowerOrUnfollowList('') : setFollowerOrUnfollowList('followers')
  }
  const setFollowingControls = (e) => {
    e.preventDefault();
    followerOrUnfollowList === 'following' ? setFollowerOrUnfollowList('') : setFollowerOrUnfollowList('following')
  }
  ...
  const [isFollowing,setIsFollowing] = useState(userProfile?.followers?.find(follower=>follower._id === user._id))
  ...
  return (
    <>
      {
        followerOrUnfollowList === 'following' ? (
          <div><FollowerFollowingList list={userProfile.following} userProfileId={userProfile._id} setFollowerOrUnfollowList={setFollowerOrUnfollowList}/></div> 
        ) : followerOrUnfollowList === 'followers' ? (
          <div><FollowerFollowingList list={userProfile.followers} userProfileId={userProfile._id} setFollowerOrUnfollowList={setFollowerOrUnfollowList} /></div>
        ) : null
      }
            ...
            <section>
              <div className='flex flex-col gap-5'>
                ...
                <div className='flex items-center gap-4'>
                  <p className='cursor-pointer hover:bg-slate-200 rounded' onClick={() => handleTabChange('posts')}><span className='font-semibold '>{userProfile?.posts.length} </span>posts</p>
                  <p className='cursor-pointer hover:bg-slate-200 rounded' onClick={(e) => setFollowersControls(e)}><span className='font-semibold'>{userProfile?.followers.length} </span>followers</p>
                  <p className='cursor-pointer hover:bg-slate-200 rounded' onClick={(e) => setFollowingControls(e)}><span className='font-semibold'>{userProfile?.following.length} </span>following</p>
                </div>
